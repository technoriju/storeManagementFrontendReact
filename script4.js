...
code = code.replace(/\{required && <Text style=\{styles\.required\}>\*<\/Text>\}/g, '');
code = code.replace(/const FormGroup = \(\{ label, required, children, actionRight, width \}: any\)/, 'const FormGroup = ({ label, children, actionRight, width }: any)');

// 3. Remove ` required` usages (only those inside JSX like `<FormGroup ... required>`)
// Actually, we can just replace ` required>` with `>` and ` required ` with ` ` and ` required\n` with `\n` etc.
// Safer way:
code = code.replace(/\s+required(\s|>)/g, '$1');
code = code.replace(/\s+required=\{true\}/g, '');
...
// 6. Fix radio group
code = code.replace(/<View style=\{styles\.radioGroup\}>[\s\S]*?<\/View>\s*<\/FormGroup>/,
