...
export const BrandsScreen: React.FC<Props> = ({ onNavigate }) => {
  const theme = useTheme();

  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Brands</Text>
        <AppButton title="Back" onPress={() => onNavigate('list')} variant="outline" />
      </View>
      <View style={styles.content}>
        <Text style={{ color: theme.colors.textSecondary }}>Brands management coming soon...</Text>
      </View>
    </View>
  );
};
